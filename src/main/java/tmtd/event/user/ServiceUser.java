...
public class ServiceUser {
    ...
    public void changePassword(Long userId, String oldPassword, String newPassword) {
        ...
        var user = jpaUser.findById(id).orElseThrow(() ->
                new org.springframework.web.server.ResponseStatusException(
                        org.springframework.http.HttpStatus.NOT_FOUND, "User not found"));

        if (oldPassword == null || newPassword == null) {
            throw new org.springframework.web.server.ResponseStatusException(
                    org.springframework.http.HttpStatus.BAD_REQUEST, "Missing password");
        }

        if (!passwordEncoder.matches(oldPassword, user.getPassword())) {
            throw new org.springframework.web.server.ResponseStatusException(
                    org.springframework.http.HttpStatus.BAD_REQUEST, "Mật khẩu cũ không đúng");
        }

        if (oldPassword.equals(newPassword)) {
            throw new org.springframework.web.server.ResponseStatusException(
                    org.springframework.http.HttpStatus.BAD_REQUEST, "Mật khẩu mới không được trùng mật khẩu cũ");
        }

        if (newPassword.length() < 8) {
            throw new org.springframework.web.server.ResponseStatusException(
                    org.springframework.http.HttpStatus.BAD_REQUEST, "Mật khẩu mới tối thiểu 8 ký tự");
        }

        user.setPassword(passwordEncoder.encode(newPassword));
        jpaUser.save(user);
        // JWT stateless: khuyến nghị FE logout + login lại để nhận token mới.
    }
    // =================== HẾT PHẦN THÊM MỚI ===================
    ...
    public UserResponse addRole(Long userId, String role) {
